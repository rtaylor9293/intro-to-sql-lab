...
-- SELECT name AS country, code AS country_code
-- FROM countries AS c
-- JOIN countrylanguages AS cl ON c.code = cl.countrycode
-- GROUP BY c.code, c.name
-- HAVING COUNT(cl.language) = 1 AND MAX(cl.language) = 'Italian';

-- Clue #4: We're booking the first flight out – maybe we've actually got a chance to catch her this time. There are only two cities she could be flying to in the country. One is named the same as the country – that would be too obvious. We're following our gut on this one; find out what other city in that country she might be flying to.

-- Write SQL query here

-- SELECT name AS city_name, countrycode
-- FROM cities
...
-- FROM cities AS ci
-- JOIN countries AS co ON ci.countrycode = co.code
...
-- SELECT ci.name AS capital , co.name AS country
-- FROM countries AS co
-- JOIN cities AS ci ON co.capital = ci.id
...
SELECT 
    ci.name AS city,
    co.name AS country,
    ci.population AS current_population,
    ci.population + 1 AS population_after_she_arrives
FROM cities ci
